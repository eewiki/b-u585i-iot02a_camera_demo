...
static CAMERA_Ctx_t Camera_Ctx[CAMERA_INSTANCES_NBR];
...
/**
  ...
  * @param  len length of the camera output buffer
...
int32_t BSP_CAMERA_Start(uint32_t Instance, uint8_t *pBff, uint32_t len, uint32_t Mode)
{
  ...
  else
  {
    if (len == 0)
    {
      len = (uint32_t)GetSize(Camera_Ctx[Instance].Resolution, Camera_Ctx[Instance].PixelFormat);
    }
    else
    {
      len >>= 2; // divide by 4 because buffer cast to uint32_t
    }

    if (HAL_DCMI_Start_DMA(hcamera_dcmi, Mode, (uint32_t)pBff, len) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else
