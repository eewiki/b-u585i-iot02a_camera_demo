...
int32_t VL53L5CX_Init(VL53L5CX_Object_t *pObj)
{
  ...
  ret = vl53l5cx_set_sharpener_percent(&pObj->Dev, 20);
...
int32_t VL53L5CX_ConfigProfile(VL53L5CX_Object_t *pObj, VL53L5CX_ProfileConfig_t *pConfig)
{
  ...
  else
  {
    ...
    pObj->RangingProfile = pConfig->RangingProfile;
...
//  }
  ...
  {
    if (pObj->RangingProfile == VL53L5CX_PROFILE_8x8_CONTINUOUS || pObj->RangingProfile == VL53L5CX_PROFILE_8x8_AUTONOMOUS)
    {
      resolution = 64;
    }
    else if (pObj->RangingProfile == VL53L5CX_PROFILE_4x4_CONTINUOUS || pObj->RangingProfile == VL53L5CX_PROFILE_4x4_AUTONOMOUS)
    {
      resolution = 16;
    }
    else
