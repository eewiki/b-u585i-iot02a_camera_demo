...
#endif
__ALIGN_BEGIN static UCHAR  camera_byte_pool_buffer[CAMERA_APP_MEM_POOL_SIZE] __ALIGN_END;
static TX_BYTE_POOL camera_app_byte_pool;

#if defined ( __ICCARM__ )
#pragma data_alignment=4
#endif
__ALIGN_BEGIN static UCHAR  tof_byte_pool_buffer[TOF_APP_MEM_POOL_SIZE] __ALIGN_END;
static TX_BYTE_POOL tof_app_byte_pool;
...
/* Set usbx_pool start address to 0x24036000 */ // TODO: should I adapt this?
...
#if (USE_STATIC_ALLOCATION == 1)
  UINT status1 = TX_SUCCESS;
  VOID *memory_ptr1;

	if (tx_byte_pool_create(&camera_app_byte_pool, "Camera App memory pool", camera_byte_pool_buffer, CAMERA_APP_MEM_POOL_SIZE) != TX_SUCCESS)
	{
		while (1);
	}
	else
	{
		memory_ptr1 = (VOID *)&camera_app_byte_pool;
		status1 = Camera_Init(memory_ptr1);
		if (status1 != TX_SUCCESS)
		{
			while (1);
		}
	}
	if (tx_byte_pool_create(&tof_app_byte_pool, "ToF App memory pool", tof_byte_pool_buffer, TOF_APP_MEM_POOL_SIZE) != TX_SUCCESS)
	{
		while (1);
	}
	else
	{
		memory_ptr1 = (VOID *)&tof_app_byte_pool;
		status1 = ToF_Init(memory_ptr1);
		if (status1 != TX_SUCCESS)
		{
			while (1);
		}
